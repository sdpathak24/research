...
def test_generator(test_path, target_size=(256, 256), batch_size=1):
    """
    Generator for test data.

    Args:
        test_path (str): Path to the test images.
        target_size (tuple): Size to resize the images (height, width).
...
#     results = model.predict(test_datagen, steps=3, verbose=1)
#     save_result(output_path, results)
#     print('Prediction on the given data done')

def model_predicting(input_path, output_path, pretrain=True):
    """Use trained model to predict predicted_mask."""
    if pretrain:
        model = solarUnet('pretrained_model/solarUnet_magnetic.hdf5')
    else:
    ...
    test_datagen = test_generator(input_path + 'test', target_size=(256, 256))
    
    # Calculate steps based on the number of test files
    num_test_files = len([f for f in os.listdir(input_path + 'test') if os.path.isfile(os.path.join(input_path + 'test', f))])
    
    # Predict dynamically or with specified steps
    results = model.predict(test_datagen, steps=num_test_files, verbose=1)
    save_result(output_path, results)
    print('Prediction on the given data done')
